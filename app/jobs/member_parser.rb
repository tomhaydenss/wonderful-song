...
class MemberParser
  DOCUMENTS_MAP = {
    rg: IdentityDocumentType::ID,
    cpf: IdentityDocumentType::TAX_PAYER,
  ...
  def identity_document(row, type)
    ...
    IdentityDocument.new(
      ...
      identity_document_type: IdentityDocumentType.find_by(description: DOCUMENTS_MAP[type]),
  ...
  def phone_type(type)
    return PhoneType.home if type.present? && type.casecmp('residencial').zero?
